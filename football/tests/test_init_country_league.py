...
from football.management.commands.init_country import Command
...
class TestInitCountryLeagueCommand:
    """Test suite for the init_country management command"""

    @patch('football.management.commands.init_country.get_league_details')
    def test_successful_initialization(self, mock_get_league_details, sample_api_response):
        ...
        out = StringIO()
        call_command('init_country', stdout=out)
    ...
    def test_custom_batch_size(self, mock_get_league_details, sample_api_response):
        ...
        out = StringIO()
        call_command('init_country', '--batch-size=2', stdout=out)
    ...
    def test_fetch_leagues_method(self, command, sample_api_response):
        ...
        with patch('football.management.commands.init_country.get_league_details') as mock_api:
